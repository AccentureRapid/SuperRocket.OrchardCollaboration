function displayPromo() {
	//display the promo code, you could get this from PHP to be more secure
						...
						FB.api('/me/feed', 'post', { message: body, link: link }, function (response) {
							if (response.error) {
								...
								if (response.error.code != 200) {
									// 200 means they denied the permission to post to their wall, don't do anything
									if (response.error.code != 506) {
										//506 means the message was posted more than once and denied, just show the discount again
