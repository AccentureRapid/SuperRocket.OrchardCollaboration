 ...
 *     return directiveDefinitionObject;
 *     // or
 *     // return function postLink(scope, Element, iAttrs) { ... }
 *   });
 * ```
 *
 *
 ...
 *
 *   * `Element` - instance element - The element where the directive is to be used. It is safe to
...
function $CompileProvider($provide, $$sanitizeUriProvider) {
  ...
  this.$get = [
      ...
      switch (nodeType) {
        case NODE_TYPE_ELEMENT: /* Element */
          ...
          for (var attr, name, nName, ngAttrName, value, isNgAttr, nAttrs = node.attributes,
            ...
            if (directiveIsMultElement(directiveNName)) {
     ...
     */
    function directiveIsMultElement(name) {
      if (hasDirectives.hasOwnProperty(name)) {
        for (var directive, directives = $injector.get(name + Suffix),
            i = 0, ii = directives.length; i < ii; i++) {
          directive = directives[i];
          if (directive.multElement) {
            return true;
          }
        }
      }
